...
(function (SlsTrSales) {
    ...
    function InitializeEvents() {
        ...
        txt_search.onkeyup = searcdisplay;
    }
    function timer() {
        debugger;
        var date = new Date();
    ...
    function searcdisplay() {
        ...
        if (txt_search.value != "") {
            ...
            var itembar = FamilyDetails.filter(function (x) { return x.PRODUCT_NAME.toLowerCase().search(search_1) >= 0; });
            if (itembar.length > 0) {
                document.getElementById("uul").innerHTML = '';
                DisplayItems(itembar);
            }
            else {
                document.getElementById("uul").innerHTML = '';
            }
    ...
    function get_cust() {
        ...
        if (cust_search_phone.value == "") {
            ...
            var search_2 = cust_search_phone.value.toLowerCase();
            SearchDetails = CustomerDetails.filter(function (x) { return x.PHONE.toString().search(search_2) >= 0 || x.CUSTOMER_NAME.toLowerCase().search(search_2) >= 0; }); /*|| x.MOBILE.toLowerCase().search(search) >= 0*/
