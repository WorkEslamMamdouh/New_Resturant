...
namespace SlsTrSales {
    ...
    var old_Family: Array<PRODUCT> = new Array<PRODUCT>();
    ...
    function searcdisplay() {
        ...
        if (txt_search.value != "") {
            ...
            var itembar = FamilyDetails.filter(x => x.PRODUCT_NAME.toLowerCase().search(search) >= 0);
            if (itembar.length > 0) {
    ...
    ////------------------------------------------------------Edit-----------------------------------
    function click_Edit(New_Name: string, New_Pirce: number, new_Qet: number, New_OnhandQty: number, New_MinUnitPrice: number, Remarks: string) {
        //debugger
